...
if (isset($_POST['enviado_edit'])) {
	...
	$model->update = 'zona';
	$model->set = "zona='$zona'";
	$model->condition = " WHERE id_zona=$id";
	$model->Update();
	$mensaje = "<script>";
	$mensaje .= "alert('Zona actualizada correctemente.');";
...
if (isset($_POST['frm_reg_zona'])) {
	
	$nombre_ap = htmlspecialchars($_POST['nombre']);

	$model = new Crud;
	$model->insertInto = 'zona';
	$model->insertColumns = 'zona';
	$model->insertValues = "'$nombre_ap'";
	$model->Create();
	$mensaje = "<script>";
	$mensaje .= "alert('La zona fue registrada correctamente');";
	$mensaje .= "document.location.href = document.location.href;";
	$mensaje .= "</script>";
	echo $mensaje;
}
...
<body>
	...
	<div id="sidebar-collapse" class="col-sm-3 col-lg-2 sidebar">
		<ul class="nav menu">
			...
			<li class="active"><a href="ad_puntos_centas.php"><span class="icon-location icono"></span>Puntos de venta</a></li>
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">			
		...
		<div class="row">
			...
			<div class="form-group">
				<button class="btn btn-primary" data-toggle="modal" data-target="#modalRegistroVendedor"><span class="icon-location icono"></span> Agregar Punto de venta</button>
			</div>
				<?php 
				if ($total10 == 0) {
				?>
				...
				<table class="table table-bordered">
					...
					<tbody>
						...
						foreach ($filas as $fila) {
							echo '<tr>';
							echo '<td>'.$fila['id_zona'].'</td>';
							echo '<td>'.$fila['zona'].'</td>';
							echo '<td>'.$fila['data'].'</td>';
							echo '<td><form class="form-horizontal" id="option_edit" method="post" action="'.$_SERVER["PHP_SELF"].'"><input type="submit" class="btn btn-danger btn_delete" style="margin-right:5px;" value="Eliminar"><input type="hidden" name="id" value="'.$fila['id_zona'].'"><input type="hidden" name="btn_delete"><button class="btn btn-warning btn_actualizar" style="margin-right:5px;" data-toggle="modal" data-target="#modalEditar'.$fila['id_zona'].'">Editar</button></form></td>';
							echo '</tr>';
						}
		...
		<!-- Modal -->
    		...
    		<div class="modal-dialog modal-sm">
      			<div class="modal-content">
        			<div class="modal-header">
          				...
          				<h4 class="modal-title">Registrar Punto de venta</h4>
        			</div>
        			<div class="modal-body">
        				<div class="container-fluid">
	          				<form class="form-horizontal" id="form_registrar_vendedor" method="post" action="<?php echo $_SERVER['PHP_SELF'];?>">
	  							<div class="form-group">
	    							<div class="col-sm-9">
	    								<input type="text" name="nombre" id="nombre" class="form-control" placeholder="Zona">
	  								</div>
	  							</div>
	  							<div class="form-group">
	    							<div class="col-sm-9">
	    								<input type="hidden" name="frm_reg_zona">
  		...
  		if ($total10 > 0) {
	  		...
	  		foreach ($filas as $fila) {
	  		?>
	  		<div class="modal fade" id="modalEditar<?php echo $fila['id_zona'];?>" role="dialog">
	        			...
	        			<div class="modal-body">
	        				<div class="container-fluid">
		          				<form class="form-horizontal" id="form_registrar_auto" method="post" action="<?php echo $_SERVER['PHP_SELF'];?>">
		  							<div class="form-group">
	    							<div class="col-sm-9">
	    								<input type="text" name="nombre" value="<?php echo $fila['zona'];?>" id="nombre" class="form-control" placeholder="Zona">
	  								</div>
	  							</div>
		  							...
		  							<div class="form-group">
		    							<div class="col-sm-9">
		    								...
		    								<input type="hidden" name="id" value="<?php echo $fila['id_zona'];?>">
